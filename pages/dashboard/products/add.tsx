...
export default function Page(){
    ...
    const sendData = async () => {
        ...
        try{
            ...
            router.replace(`/dashboard/products/${res.data}`)
        }
        catch(e){
    ...
    return(
        <WithAuth withPermission='products'>
            <Head title='ProdBoost - Dodaj nowy produkt'/>
            <Navbar/>
            <LoadingOverlay visible={sendingData} sx={{position:'fixed'}}/>
            <Container>
                <Navigation items={items}/>
                <Title order={1} mb={20} align='center'>Dodaj nowy produkt</Title>

...
const items = [
    ...
    { title: 'Opakowania', href: '/dashboard/packing' },
    { title: 'Dodaj nowe opakowanie', href: ''}
]
...
export default function Page(){
    ...
    const sendData = async () => {
        setSendingData(true);
        try{
            const res = await axios.post('/api/packing/add', form)
            router.replace(`/dashboard/packing/${res.data}`)
        }
        catch(e){
    ...
    return(
        <WithAuth withPermission='packing'>
            <Head title='ProdBoost - Dodaj nowy opakowanie'/>
            <Navbar/>
            <LoadingOverlay visible={sendingData} sx={{position:'fixed'}}/>
            <Container>
                <Navigation items={items}/>
                <Title order={1} mb={20} align='center'>Dodaj nowe opakowanie</Title>
                <Group position='center' direction='column' grow mx='auto' sx={{maxWidth: '400px'}}>
                    ...
                    <Tooltip
                        ...
                        radius='md'
                        wrapLines
                        label='Opakowania można wyszukiwać za pomocą nazwy lub tagów. Możesz użyć tagów w celu kategoryzacji przedmiotu np. Deska drewniana - tagi: "olchowa", "szlifowana".'
                        width={250}
                    >
                        <TextInput 
                            rightSection={

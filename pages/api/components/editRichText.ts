...
export default async (req: NextApiRequest, res: NextApiResponse) => {
    if(req.method === 'POST'){
        ...
        try{
            const currentComponent = await db.collection('components').findOne({_id: new ObjectId(componentId)}, {projection: {_id: 0, richTextData: 1}})
            const currentRichTextData: string = currentComponent?.richTextData;

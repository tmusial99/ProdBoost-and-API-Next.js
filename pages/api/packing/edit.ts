...
export default async (req: NextApiRequest, res: NextApiResponse) => {
    if(req.method === 'POST'){
        const {data, packingId}: {data: IPacking, packingId: string} = req.body;
        if(!data || !data.name || data.quantity as number < 0 || !packingId){
        ...
        try{
            await db.collection('packing').updateOne({_id: new ObjectId(packingId)}, {

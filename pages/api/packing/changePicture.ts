...
export default async (req: NextApiRequest, res: NextApiResponse) => {
    if(req.method === 'POST'){
        ...
        const packing = await db.collection('packing').findOne({_id: new ObjectId(componentId)})
        ...
        const params: PutObjectCommandInput = {
            Bucket: 'prodboost',
            Key: `packing_images/${Date.now()}${getRandomInt(100,1000)}.webp`,
        ...
        const response = await db.collection('packing').updateOne(

...
export default async (req: NextApiRequest, res: NextApiResponse) => {
    if(req.method === 'POST'){
        ...
        const component = await db.collection('products').findOne({_id: new ObjectId(componentId)})
        ...
        const params: PutObjectCommandInput = {
            Bucket: 'prodboost',
            Key: `products_images/${Date.now()}${getRandomInt(100,1000)}.webp`,
        ...
        if(component?.image_url){
            await s3client.send(new DeleteObjectCommand({
                Bucket: 'prodboost',
                Key: `products_images/${(component.image_url as string).split('/').pop()}`
            }))
        }
